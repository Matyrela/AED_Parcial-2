...
public class UtilGrafos {
    ...
    public static Boolean[][] obtenerMatrizAdy(Map<Comparable, TVertice> vertices) {
        int cantidadVertices = vertices.size();
        Boolean[][] matrizCostos = new Boolean[cantidadVertices][cantidadVertices];

        for (int i = 0; i < matrizCostos.length; i++) {
            for (int j = 0; j < matrizCostos.length; j++) {
               matrizCostos[i][j] = false;
        ...
        while (i < cantidadVertices) {
            ...
            while (j < cantidadVertices) {
                ...
                if (!elemVerticeI.getEtiqueta().equals(elemVerticeJ.getEtiqueta())) {
                    TVertice verticeI = (TVertice) elemVerticeI;
                    TVertice verticeJ = (TVertice) elemVerticeJ;
                    TAdyacencia Adyacencia = verticeI.buscarAdyacencia(verticeJ);
                    if (Adyacencia != null){
                        matrizCostos[i][j] = true;
                    }
                }
                j++;
            }
    ...
    public static void imprimirMatrizAdyMejorado(boolean[][] matriz, Map<Comparable, TVertice> vertices, String titulo) {
        if (vertices != null && matriz.length == vertices.keySet().size()) {
            ...
            for (int i = 0; i < matriz.length; i++) {
                ...
                for (int j = 0; j < matriz.length; j++) {
                    ...
                    if (!matriz[i][j]) {
                        ...
                        System.out.print(rellenar(adyacente, largo, ' '));
                    }
                }
                System.out.println();
                System.out.println(linea);
